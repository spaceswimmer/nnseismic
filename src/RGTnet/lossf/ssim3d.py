...
def _ssim(input, target, max_val, k1, k2, channel, kernel):
    ...
    mu1 = F.conv2d(input, kernel, groups=channel)
    ...
    sigma1_sq = F.conv2d(input * input, kernel, groups=channel) - mu1_sq
    sigma2_sq = F.conv2d(target * target, kernel, groups=channel) - mu2_sq
    sigma12 = F.conv2d(input * target, kernel, groups=channel) - mu1_mu2
...
def ms_ssim_loss3d(input, target, max_val, filter_size=11, k1=0.01, k2=0.03,
                 sigma=1.5, kernel=None, weights=None, size_average=None, reduce=None, reduction='mean'):
