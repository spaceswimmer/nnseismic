...
tensorboard_log_path = os.path.join(opt.sessions_path, ".logs", session_name)
...
def train_model(
    ...
    for epoch in range(num_epochs):
        train_epoch_loss_ifo = train(
            model, dataloader, optimizer, criterion, scheduler, epoch, opt
        )
        train_loss_history.append(train_epoch_loss_ifo)

        if writer is not None:
            loss_name = criterion.getLossName()
            writer.add_scalar(
                f"train/{loss_name}", train_epoch_loss_ifo[loss_name], epoch
            )
            writer.add_scalar("train/LR", train_epoch_loss_ifo["LR"], epoch)
        ...
        if opt.valid and epoch % opt.valid_interval == 0:
            ...
            if writer is not None:
                writer.add_scalar("valid/MAE", test_epoch_loss_ifo["MAE"], epoch)
                writer.add_scalar("valid/MSE", test_epoch_loss_ifo["MSE"], epoch)
                writer.add_scalar("valid/RMSE", test_epoch_loss_ifo["RMSE"], epoch)
                writer.add_scalar("valid/MRPD", test_epoch_loss_ifo["MRPD"], epoch)
                writer.add_scalar("valid/SSIM", test_epoch_loss_ifo["SSIM"], epoch)

            if test_epoch_loss_ifo["SSIM"] < best_val_loss:
                best_val_loss = test_epoch_loss_ifo["SSIM"]
        ...
        if epoch % opt.history_interval == 0:
            utils.save_training_history(
                train_loss_history, os.path.join(history_path, f"train_history.txt")
            )
            if opt.valid:
                utils.save_training_history(
                    test_loss_history, os.path.join(history_path, f"test_history.txt")
                )
...
def valid(model, dataloader, criterion, epoch, opt):
    ...
    tqdm.write("=> Valid Epoch:{0} Loss={Loss:.5f}".format(epoch, Loss=epoch_loss))
    tqdm.write(
        "=> RGT:"
...
def draw(seis, rgt, rgt_pre, batch_index, epoch=0, sec_idx=None):
    ...
    if epoch == 0:
        ...
        utils.draw_img(
            ...
            cmap="jet",
            path=os.path.join(
                path_label_epoch, f"rgt_{epoch}_{batch_index}_{sec_idx}.png"
            ),
        )
        utils.draw_img(
            seis2d,
            ...
            cmap="gray",
            path=os.path.join(
                path_label_epoch, f"seis_{epoch}_{batch_index}_{sec_idx}.png"
            ),
        )
...
def init_run():
    ...
    utils.makeDir(tensorboard_log_path)
    opt_dict = json.dumps(
        vars(opt), indent=4, separators=(",", ":"), ensure_ascii=False
    )
...
if __name__ == "__main__":
    ...
    dataloader = DataLoader(
        dataset, batch_size=batch_size, shuffle=True, num_workers=opt.num_workers
    )
    dataloader_val = None
