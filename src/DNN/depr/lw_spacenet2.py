...
from .rgt_loss import CombinedLoss
import numpy as np
import os
...
class SeismicTrainer:
    def __init__(self, model, train_loader, val_loader, device='cuda'):
        ...
        # Use paper's SSIM loss (start with SSIM only to test)
        self.criterion = CombinedLoss(
            lambda_ssim=0.5,    # Start with SSIM only
            lambda_mse=0.5,     # Set to 0.5 to combine with MSE
            channel=1
        )
    ...
    def train_epoch(self):
        ...
        for batch_idx, (seismic, rgt) in enumerate(self.train_loader):
            ...
            loss, loss_dict = self.criterion(outputs, rgt)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for seismic, rgt in self.val_loader:
                ...
                outputs = self.model(seismic)
                loss, _ = self.criterion(outputs, rgt)
